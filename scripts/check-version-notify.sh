...
if [ "$CURRENT_VERSION" = "$LAST_VERSION" ]; then
  # ---- 无新版本：发简短日报 ----
  FEISHU_MSG="📋 OpenClaw 每日版本检查｜${TODAY}
...
要求：
- 分「重要变更」「新功能」「问题修复」三个分类（没有的类别可省略）
- 每条一行，用「・」开头
- 保留关键技术术语（如 Telegram、Matrix、Feishu 等）
- 总字数控制在 300 字以内
- 只输出摘要内容，不要其他说明" 2>/dev/null || echo "（摘要生成失败，请查看原始 changelog）")

  FEISHU_MSG="🆕 OpenClaw 新版本发布！｜${TODAY}

新版本：${CURRENT_VERSION}
上一版本：${LAST_VERSION:-首次检查}

${SUMMARY}

📌 完整 changelog：docs.openclaw.ai/changelog"
